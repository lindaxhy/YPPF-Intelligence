...
def changeActivityStatus(aid, cur_status, to_status):
    ...
    # 如果当前状态与期望状态不符，尝试自动状态跳跃
    if cur_status != activity.status:
        # 定义状态转换路径
        FSM = [
            (Activity.Status.UNPUBLISHED, Activity.Status.APPLYING),
        ...
        # 构建状态转换图
        status_graph = {}
        for from_status, to_status in FSM:
            if from_status not in status_graph:
                status_graph[from_status] = []
            status_graph[from_status].append(to_status)
        ...
        def find_path(current, target, current_path):
            ...
            if current in status_graph:
                for next_status in status_graph[current]:
                    result = find_path(next_status, target,
                                       current_path + [next_status])
                    if result:
                        return result
            return None

        path = find_path(current_status, target_status, [])

        if path:
            # 找到路径，执行状态跳跃
            logger.info(f'活动 {aid} 状态跳跃: {current_status} -> {path}')

            # 执行状态跳跃，跳过中间状态的处理逻辑
            for next_status in path:
                # 只执行必要的状态转换逻辑
                if next_status == Activity.Status.WAITING:
                    if activity.bidding:
                        draw_lots(activity)
                elif next_status == Activity.Status.PROGRESSING:
                ...
                elif (next_status == Activity.Status.END
                        and activity.category != Activity.ActivityCategory.COURSE):
                    activity.settle_yqpoint(status=next_status)
            ...
            # 无法找到路径，抛出异常
            assert cur_status == activity.status, f'<{activity.status}>与期望的状态<{cur_status}>不符，且无法进行状态跳跃'
...
def draw_lots(activity: Activity):
    ...
    sender = activity.organization_id.get_user()  # type: ignore
...
def _subscriber_uids(activity: Activity) -> list[int]:
    return SQ.qsvlist(Person.objects.activated().exclude(
        id__in=activity.organization_id.unsubscribers.all()  # type: ignore
    ), Person.person_id)
...
def _set_change_status(activity: Activity, current, next, time, replace):
    ScheduleAdder(changeActivityStatus, id=f'activity_{activity.id}_{next}',  # type: ignore
                  run_time=time, replace=replace)(activity.id, current, next)  # type: ignore


def _set_jobs_to_status(activity: Activity, replace: bool) -> Activity.Status:
    now_time = datetime.now()
    status = Activity.Status.END
    if now_time < activity.end:  # type: ignore
        status, next = Activity.Status.PROGRESSING, Activity.Status.END
        _set_change_status(activity, status, next, activity.end, replace)
    if now_time < activity.start:  # type: ignore
    ...
    if now_time < activity.start - timedelta(minutes=15):  # type: ignore
        reminder = ScheduleAdder(notifyActivity, id=f'activity_{activity.id}_remind',
                                 run_time=activity.start - timedelta(minutes=15), replace=replace)  # type: ignore
        reminder(activity.id, 'remind')  # type: ignore
    return status
...
def create_activity(request):
    ...
    if context.get("template_id"):
        ...
        photo = ActivityPhoto.objects.get(  # type: ignore
        ...
        ActivityPhoto.objects.create(  # type: ignore
...
def apply_activity(request, activity: Activity):
    ...
    if not activity.bidding:
        if activity.current_participants < activity.capacity:
            activity.current_participants += 1  # type: ignore
        else:
...
def _update_new_participants(activity: Activity, new_participant_uids: list[str]) -> int:
    ...
    create_participate_infos(
        activity, new_participant_nps, status=status)  # type:ignore
...
def modify_participants(activity: Activity, new_participant_uids: list[str]):
    ...
    activity.current_participants += _update_new_participants(
        activity, new_uids)  # type: ignore
    activity.save()
